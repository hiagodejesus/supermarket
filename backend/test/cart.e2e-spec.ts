...
describe('Cart (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    await postgresService.client.query('TRUNCATE carts, cart_items RESTART IDENTITY CASCADE');
  ...
  it('should remove a product from the cart if quantity is 0', async () => {
    ...
    const response2 = await request(app.getHttpServer()).delete(`/cart/${response.body.id}/items/1`);
    expect(response2.status).toBe(200);
