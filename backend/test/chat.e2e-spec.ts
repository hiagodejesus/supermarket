...
describe('Chat (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    await postgresService.client.query('TRUNCATE chat_sessions RESTART IDENTITY CASCADE');
  ...
  it('should create a new chat session', async () => {
    ...
    const getResponse = await request(app.getHttpServer()).get(`/chat/${response.body.id}`);
  ...
  it('should add new messages to the chat session', async () => {
    ...
    const sessionId = response.body.id as number;

    const messageResponse = await request(app.getHttpServer())
      .post(`/chat/${sessionId}/messages`).send({
        content: 'Hello world'
      });

    expect(messageResponse.status).toBe(201);
    expect(messageResponse.body).toHaveProperty('id');
    expect(messageResponse.body).toHaveProperty('created_at');
    expect(messageResponse.body).toHaveProperty('content', 'Hello world');
    expect(messageResponse.body).toHaveProperty('chat_session_id', sessionId);
    ...
    expect(getResponse.body.messages[0]).toHaveProperty('message_type', 'text');
    expect(getResponse.body.messages[1]).toHaveProperty('sender', 'assistant');
  });

  
  it('should add new messages to the chat session with actions', async () => {
    const response = await request(app.getHttpServer()).post('/chat');
    expect(response.status).toBe(201);
    expect(response.body).toHaveProperty('id');
    ...
    expect(getResponse.body.messages).toBeDefined();
    expect(getResponse.body.messages[0]).toHaveProperty('sender', 'user');
    expect(getResponse.body.messages[0]).toHaveProperty('content', 'Quero preparar um bolo de chocolate');
    expect(getResponse.body.messages[0]).toHaveProperty('message_type', 'text');
    expect(getResponse.body.messages[1]).toHaveProperty('sender', 'assistant');
    expect(getResponse.body.messages[1]).toHaveProperty('action');

    const postConfirmResponse = await request(app.getHttpServer()).post(`
      /chat/${sessionId}/action/${getResponse.body.messages[1].action.id}/confirmactions}
      `);
