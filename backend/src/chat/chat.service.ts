...
export class ChatService {
    constructor(private readonly postgresService: PostgresService, private readonly llmService: LlmService) {}

    async createChatSession(userId: number) {
        const result = await this.postgresService.client.query<{id: number}>(
            `INSERT INTO chat_sessions (user_id) VALUES ($1) RETURNING *`, [userId]
        );
            ...
            COALESCE(
                json_agg(
                    json_build_object(
                        ...
                        'content', cm.content,
                        'sender', cm.sender,
                        'openai_message_id', cm.opeai_message_id,
    ...
    async getChatSession(sessionId: number){
        const result = await this.postgresService.client.query<{id: number, created_at: Date, user_id: number, messages: ChatMessage[] | null}>(
            `SELECT chat_sessions.id, chat_sessions.created_at, chat_sessions.user_id, JSON_AGG(
             JSON_BUILD_OBJECT(
              ...
              'action', CASE
                WHEN chat_messages_actions.id IS NOT NULL THEN JSON_BUILD_OBJECT(
        ...
        const populatedMessages = await this.populateMessages(result.rows[0].messages ?? []);
        return {
            ...result.rows[0],
            messages: populatedMessages.sort((a,b) => a.created_at.getTime() - b.created_at.getTime()) ?? []
        }
    }
    ...
    async addUserMessage(sessionId: number, content: string) {
        ...
        const userMessage =  this.addMessageToSession(sessionId, content, 'user');
    ...
    async confirmAction(sessionId: number, actionId: number) {
        const session = await this.postgresService.client.query(
        ...
        if(result.rows[0].action_type === 'suggest_carts') {
            ...
            if (!embeddings) {
                throw new BadGatewayException('Failed to embed input');
            }

            const mostSimilarProducts = await this.postgresService.client.query<SimilarProducts>(
                `
            ...
            if (mostSimilarProducts.rows.length === 0) {
                throw new NotFoundException('No similar products found');
            }

            const llmResponse = await this.llmService.suggestCarts(mostSimilarProducts.rows, result.rows[0].payload.input);
            ...
            await this.saveSuggestedCart(message.id, llmResponse.carts);
    ...
    private async saveSuggestedCart(
        messageId: number, carts: {
            store_id: number; 
            score: number;
            products: {
                id: number;
            ...
            for (const cart of carts) {
                const cartResult = await this.postgresService.client.query<{
                    ...
                    `INSERT INTO carts (user_id, store_id, score, suggested_by_message_id, active) VALUES ($1, $2, $3, $4, TRUE) RETURNING id`,
