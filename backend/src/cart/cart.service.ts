...
export class CartService {
    ...
    async addToCart(userId: number, productId: number, quantity: number) {
        const product = await this.postgresService.client.query<{ store_id: number }>(
        ...
        const existingCart = await this.postgresService.client.query<{id: number, store_id: number}>(
        ...
        const storeId = product.rows[0].store_id;
    ...
    async getCart(userId: number) {
        ...
        return {
            ...
            total: result.rows[0].items?.reduce(
    ...
    async updateCartItemQuantity(
        ...
        await this.postgresService.client.query<Cart>(
